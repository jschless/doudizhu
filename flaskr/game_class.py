...
from .game import send_socket, flash_message
...
class Game:
    ...
    def add_player_to_db(self, user, session, request):
        self.players = [p for p in self.players if p['username'] != user.username]
    ...
    def update(self):
        ...
        for p in self.players:
            send_socket('update gameboard', self.generate_game_data(p), p['socketid'])
        return get_db().ddz.games.replace_one({'game_id': self.game_id}, dict(self))
